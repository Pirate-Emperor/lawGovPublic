...
# local
import deployment_utils as dpu
from deployment_utils import  generate_random_sample, generate_highlighted_words, extract_case_information


# instantiate `DataPreparator` & `Predictor` objects
data_preparator = dpu.DataPreparator()
predictor = dpu.Predictor()
...
with open("D:\Projects\APEX_PROJECT\Ongoing\GUXLocal\code\lawGov\src\style.css") as f:
    ...
    with st.expander(label="Case Summarizer", expanded=True):
        ...
        if option == "Upload a File":
            ...
            if uploaded_file is not None:
                ...
                petitioner, respondent, case_facts = extract_case_information(content)
...
with right_col:
    ...
    with st.expander(label="Case Outcome Predictor", expanded=True):
        ...
        option = st.selectbox(
            "Select Model",
            ("Ensemble RX", "Ensemble (Doc2Vec + TF-IDF)", "Ensemble (Doc2Vec + TF-IDF)"
             "Ensemble (Doc2Vec + CNN)", "Ensemble (Doc2Vec + GloVe)", "LSTM",
             "TF-IDF", "1D Convolutional", "GloVe", "BERT", "Doc2Vec", "FastText")
        )
        ...
        if submitted:
            if petitioner and respondent and facts:
                with st.spinner("Analyzing Case Facts ..."):
                    ...
                    if option == "Doc2Vec":
                    ...
                    elif option == "Ensemble (Doc2Vec + CNN)":
                        doc2vec_predictions = predictor.predict_doc2vec(facts)
                        cnn_predictions = predictor.predict_cnn(facts)
                        ...
                        cnn_output = explainer.explain_instance(
                            facts, predictor.predict_cnn)
                        cnn_important_words = cnn_output.as_list()

                        important_words = doc2vec_important_words + cnn_important_words

                    elif option == "Ensemble (Doc2Vec + GloVe)":
                        doc2vec_predictions = predictor.predict_doc2vec(facts)
                        glove_predictions = predictor.predict_glove(facts)
                        ...
                        glove_output = explainer.explain_instance(
                            facts, predictor.predict_glove)
                        glove_important_words = glove_output.as_list()

                        important_words = doc2vec_important_words + glove_important_words
                    
                    elif option == "Ensemble RX":
                        doc2vec_predictions = predictor.predict_doc2vec(facts)
                        tf_idf_predictions = predictor.predict_tf_idf(facts)
                        cnn_predictions = predictor.predict_cnn(facts)
                        glove_predictions = predictor.predict_glove(facts)

                        predictions = (doc2vec_predictions+tf_idf_predictions+cnn_predictions+glove_predictions) / 4

                        doc2vec_output = explainer.explain_instance(facts, predictor.predict_doc2vec)
                        doc2vec_important_words = doc2vec_output.as_list()

                        tf_idf_output = explainer.explain_instance(facts, predictor.predict_tf_idf)
                        tf_idf_important_words = tf_idf_output.as_list()

                        cnn_output = explainer.explain_instance(facts, predictor.predict_cnn)
                        cnn_important_words = cnn_output.as_list()

                        glove_output = explainer.explain_instance(facts, predictor.predict_glove)
                        glove_important_words = glove_output.as_list()

                        important_words = doc2vec_important_words + tf_idf_important_words + cnn_important_words + glove_important_words
